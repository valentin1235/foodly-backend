...
for i in range(1, last_page):
    ...
    for i in unit_urls:
        ...
        # get big_image1(background image) url
        try:
            big_img = soup.select('ul.slides > li > img')
            big_imgs1.append('https:' + big_img[0]['src'])
        except:
            big_img = ''
            big_imgs1.append(big_img)
        # get big_image2 url
        try:
            big_img = soup.select('ul.slides > li > img')
            big_imgs2.append('https:' + big_img[1]['src'])
        except:
            big_img = ''
            big_imgs2.append(big_img)
        # get big_image3 url
        try:
            big_img = soup.select('ul.slides > li > img')
            big_imgs3.append('https:' + big_img[2]['src'])
        except:
            big_img = ''
            big_imgs3.append(big_img)
        # get vitamin info
        try:
            vitamin = soup.select_one(
                '#container > div.parallax__base > div:nth-child(4) > div > div > div > div > div:nth-child(2) > ul').text
        ...
        # get mineral info
        try:
            mineral = soup.select_one(
                '#container > div.parallax__base > div:nth-child(4) > div > div > div > div > div:nth-child(3) > ul').text
...
df['big_imgs1'] = big_imgs1
df['big_imgs2'] = big_imgs2
df['big_imgs3'] = big_imgs3
df['energys'] = energys
...
df.to_csv("./foodly20200302.csv", encoding='utf8')
